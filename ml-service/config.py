...
WQ_PREDICTION_INTERVAL = int(os.getenv("WQ_INTERVAL", 10))  # seconds (responsive updates)
...
FF_PREDICTION_INTERVAL = int(os.getenv("FF_INTERVAL", 10))  # seconds (responsive updates)
...
GAS_PREDICTION_INTERVAL = int(os.getenv("GAS_INTERVAL", 10))  # seconds (responsive updates)
...
STREAM_FPS = int(os.getenv("STREAM_FPS", 24))            # Target MJPEG stream FPS
YOLO_INPUT_SIZE = int(os.getenv("YOLO_SIZE", 416))        # Smaller = faster inference
OVERLAY_PERSISTENCE = float(os.getenv("OVERLAY_PERSIST", 2.0))  # Seconds to show last detection boxes
